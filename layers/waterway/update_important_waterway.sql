...
INSERT INTO osm_important_waterway_linestring (geometry, name, name_et, name_en, name_sv, tags)
...
FROM (
         ...
         WHERE name <> ''
           AND waterway = 'river'
           AND ST_IsValid(geometry)
         GROUP BY name, name_et, name_en, name_sv, slice_language_tags(tags)
     ) AS waterway_union;
...
BEGIN
    ...
    INSERT INTO osm_important_waterway_linestring_gen_z11 (geometry, id, name, name_et, name_en, name_sv, tags)
    ...
    INSERT INTO osm_important_waterway_linestring_gen_z10 (geometry, id, name, name_et, name_en, name_sv, tags)
    ...
    INSERT INTO osm_important_waterway_linestring_gen_z9 (geometry, id, name, name_et, name_en, name_sv, tags)
...
BEGIN
    IF (tg_op IN ('DELETE', 'UPDATE')) AND OLD.name <> '' AND OLD.waterway = 'river' THEN
        INSERT INTO waterway_important.changes(is_old, name, name_et, name_en, name_sv, tags)
        VALUES (TRUE, OLD.name, OLD.name_et, OLD.name_en, OLD.name_sv, slice_language_tags(OLD.tags));
    END IF;
    IF (tg_op IN ('UPDATE', 'INSERT')) AND NEW.name <> '' AND NEW.waterway = 'river' THEN
        INSERT INTO waterway_important.changes(is_old, name, name_et, name_en, name_sv, tags)
        VALUES (FALSE, NEW.name, NEW.name_et, NEW.name_en, NEW.name_sv, slice_language_tags(NEW.tags));
    END IF;
    RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    SELECT DISTINCT ON (name, name_et, name_en, name_sv, tags)
        name,
        name_et,
        name_en,
        name_sv,
    ...
    WHERE w.name = c.name
      AND w.name_et IS NOT DISTINCT FROM c.name_et
    ...
    FROM (
             ...
             FROM osm_waterway_linestring AS w
                     ...
                     w.name = c.name AND
                     w.name_et IS NOT DISTINCT FROM c.name_et AND
             ...
             WHERE w.name <> ''
               ...
               AND ST_IsValid(geometry)
             GROUP BY w.name, w.name_et, w.name_en, w.name_sv, slice_language_tags(w.tags)
         ) AS waterway_union;
