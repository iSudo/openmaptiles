CREATE OR REPLACE FUNCTION transportation_name_tags(geometry geometry, tags hstore, name text, name_en text, name_sv text) RETURNS hstore AS
$$
SELECT hstore(string_agg(nullif(slice_language_tags(tags ||
                     hstore(ARRAY [
                       ...
                       'name:de', CASE WHEN length(name_sv) > 15 THEN osml10n_street_abbrev_de(name_sv) ELSE NULLIF(name_sv, '') END
                     ]))::text,
                     ''), ','));
